...
file_trace () 
{
    ...
    do
        ...
        if [[ "$LOGGED" == "Y" ]]; then
            echo -n "$1 -warmup_instructions 5000000 -simulation_instructions"
            echo " 20000000 -traces $i > $OUT/$3---$trace 2>>$LOG"
        else
            echo -n "$1 -warmup_instructions 5000000 -simulation_instructions"
            echo " 20000000 -traces $i > $OUT/$3---$trace 2>/dev/null"
        fi
    done
}
...
file_4core_trace () 
{
    ...
    do
        ...
        if [[ "$LOGGED" == "Y" ]]; then
            echo -n "$1 -warmup_instructions 5000000 -simulation_instructions"
            echo " 20000000 -traces $trace > $OUT/$3.out---$idx.out 2>>$LOG"
        else
            echo -n "$1 -warmup_instructions 5000000 -simulation_instructions"
            echo " 20000000 -traces $trace > $OUT/$3.out---$idx.out 2>/dev/null"
        fi
        idx=$(($idx + 1))
        trace=""
    done < $2
...
print_help ()
{
    ...
    echo " -f: execute  CloudSuite, and Multi-Level prefetcher" 
...
#----------------------------------------------------------------------------#
    echo -n "Removing unicode from outputs... "
        ./format_cloudsuite.sh output/cloudsuite
    echo " ${GREEN}done${NC}"
    
    echo -n "Parsing CloudSuite... "
    if [[ "$VERBOSE" == "Y" ]]; then
        python3 Python/get_data_by_traces.py n SpeedUp output/cloudsuite > single1.csv
        python3 Python/parse_cs.py single1.csv > cs.csv
    elif [[ "$LOGGED" == "Y" ]]; then
        python3 Python/get_data_by_traces.py n SpeedUp output/cloudsuite > single1.csv 2>>$LOG
        python3 Python/parse_cs.py single1.csv > cs.csv 2>>$LOG
    else
        python3 Python/get_data_by_traces.py n SpeedUp output/cloudsuite > single1.csv 2>/dev/null
        python3 Python/parse_cs.py single1.csv > cs.csv 2>/dev/null
    fi
    echo " ${GREEN}done${NC}"
